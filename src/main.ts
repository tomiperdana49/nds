...
async function sendHsmMetaMesaageLink(phoneNumber: string, body: any, phone_number_id: any): Promise<any> {
  try {
    ...
    const headers = {
      ...
      'X-Api-Key': 'FO8yl0w2emn7rMNme3GBb2RPVa69JIdW',
    };
...
async function sendHsmMeta(phoneNumber: string, params: any, nameDoc: any, namaPic: any): Promise<any> {
  try {
    // const apiUrl = 'https://nwc.nusa.net.id/api/messages';
    // const apiUrl = 'https://nwc.nusa.net.id/api/messages?phone_number_id=122096900054011533&no_save=1';
    const apiUrl = 'https://nwc.nusa.net.id/api/messages?phone_number_id=202063559668129&no_save=1'; // ini dikirim dari no xl
...
async function sendHsmMetaRejected(phoneNumber: string, nameDocs: any, reason: any, nameRejected: any): Promise<any> {
  try {
    const apiUrl = 'https://nwc.nusa.net.id/api/messages?phone_number_id=202063559668129&no_save=1';
...
async function sendHSM(phoneNumber: string, params: any): Promise<any> {
    const ACCESS_KEY = "zb8HNFZyelPcCGHhEH7HMvT4G"; // Use environment variables to store sensitive data.
...
app.post("/get-send-link", async (req, res) => {
  try {
    ...
    if (!token || token !== '') {
